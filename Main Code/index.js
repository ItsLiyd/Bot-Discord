...
if (process.env.MONGO_URI) {
  ...
  console.warn('MONGO_URI tidak ditemukan di .env — melewati koneksi MongoDB.');
}
...
if (serverStatsCommand && serverStatsCommand.data) {
  if (!client.commands.has(serverStatsCommand.data.name)) {
...
client.on('ready', async () => {
  ...
  // Auto messages / welcomer (tetap dipanggil biar fitur lama tetap berjalan)
  autoChat(client);
  welcomer(client);
});
...
client.on('messageCreate', async (message) => {
  ...
  // pas si user tukang ilang balek: hapus cache & DB, restore nickname
  if (afkInfo) {
    ...
    setTimeout(() => reply.delete().catch(e => console.error("😹 Gagal hapus pesan 'welcome back':", e)), 10000); // menghapus pesan dalam waktu 10000ms (10 detik)
  }
  ...
  message.mentions.users.forEach(user => {
    ...
    if (afkMentioned) {
      message.reply(`**${user.username}** lagi AFK bejir\n**Alasan:** ${afkMentioned.reason}`);
    }
  });
});
...
process.on('SIGINT', async () => {
  console.log('SIGINT received — menutup client & mongoose.');
